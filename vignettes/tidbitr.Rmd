...
- Clipboard functions `ClipbrdRead` and `ClipbrdWrite` to read and write to clipboard
...
- `XmlGetValues` gets text of all XML nodes
...
```{r}
census %>%
  ClipbrdWrite(quote = F) 
```

Similarly `ClipbrdRead` can be used to read dataframe contents from the clipboard.
